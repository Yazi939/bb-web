...
function setupIPC() {
  ...
  ipcMain.handle('check-for-updates', () => {
    electronLog.info('Обработчик check-for-updates вызван');
    return autoUpdater.checkForUpdates();
  });
  ipcMain.handle('download-update', () => {
    electronLog.info('Обработчик download-update вызван');
    return autoUpdater.downloadUpdate();
  });
  ipcMain.handle('install-update', () => {
    electronLog.info('Обработчик install-update вызван');
...
function setupUpdater(mainWindow) {
  autoUpdater.on('checking-for-update', () => {
    electronLog.info('Событие: checking-for-update');
    mainWindow.webContents.send('update-status', 'checking');
  });
  autoUpdater.on('update-available', (info) => {
    electronLog.info('Событие: update-available', info);
    mainWindow.webContents.send('update-available', info);
  });
  autoUpdater.on('update-not-available', (info) => {
    electronLog.info('Событие: update-not-available', info);
    mainWindow.webContents.send('update-not-available', info);
  });
  autoUpdater.on('error', (err) => {
    electronLog.error('Событие: error', err);
    mainWindow.webContents.send('update-error', err.message);
  });
  autoUpdater.on('download-progress', (progressObj) => {
    electronLog.info('Событие: download-progress', progressObj);
    mainWindow.webContents.send('download-progress', progressObj);
  });
  autoUpdater.on('update-downloaded', (info) => {
    electronLog.info('Событие: update-downloaded', info);
    mainWindow.webContents.send('update-downloaded', info);
  });
  // Проверяем наличие обновлений при запуске
  autoUpdater.checkForUpdates().catch(err => {
    electronLog.error('Ошибка при проверке обновлений:', err);
  });
}
...
function createWindow() {
  ...
  // Добавляем инициализацию автообновлений
