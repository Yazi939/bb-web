    ...
    .reduce((sum, expense) => sum + (expense.amount || 0), 0);
      ...
      <div className={styles.tableContainer}>
        <div className={styles.tableHeader}>
          <h3 className={styles.tableTitle}>Список расходов</h3>
          <Button
            type="primary"
            icon={<PlusOutlined />}
          ...
          {(expenses || []).map((expense) => (
            <div key={expense.id} className={styles.expenseCard}>
              <div className={styles.expenseCardHeader}>
                <span className={styles.expenseCardTitle}>{expense.description}</span>
                <div className={styles.expenseCardActions}>
                  <Button size="small" icon={<EditOutlined />} onClick={() => handleEdit(expense)} />
                  <Button size="small" danger icon={<DeleteOutlined />} onClick={() => handleDelete(expense.id)} />
                </div>
              </div>
              <div className={styles.expenseCardContent}>
                <div className={styles.expenseCardField}>
                  <span className={styles.expenseCardLabel}>Сумма:</span>
                  <span className={`${styles.expenseCardValue} ${styles.amountText}`}>
                    {expense.amount.toFixed(2)} ₽
                  </span>
                </div>
                <div className={styles.expenseCardField}>
                  <span className={styles.expenseCardLabel}>Тип:</span>
                  <span className={styles.expenseCardValue}>
                    <Tag className={styles.typeTag} color={expense.type === 'fuel' ? 'blue' : expense.type === 'maintenance' ? 'orange' : 'green'}>
                      {expense.type === 'fuel' ? 'Топливо' : expense.type === 'maintenance' ? 'Обслуживание' : 'Прочее'}
                    </Tag>
                  </span>
                </div>
                <div className={styles.expenseCardField}>
                  <span className={styles.expenseCardLabel}>Дата:</span>
                  <span className={styles.expenseCardValue}>
