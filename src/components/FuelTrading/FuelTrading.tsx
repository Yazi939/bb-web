...
import styles from './FuelTrading.module.css';
                  ...
                  <div key={transaction.id} className={styles.mobileCard}>
                    <div className={styles.mobileCardHeader}>
                      <span className={styles.mobileCardTitle}>
                        {FUEL_TYPES.find(f => f.value === transaction.fuelType)?.label || transaction.fuelType}
                      </span>
                      <div className={styles.mobileCardActions}>
                        <Button size="small" icon={<EditOutlined />} onClick={() => handleEdit(transaction)} />
                        <Button size="small" danger icon={<DeleteOutlined />} onClick={() => handleDelete(transaction.id)} />
                      </div>
                    </div>
                    <div className={styles.mobileCardContent}>
                      <div className={styles.mobileCardField}>
                        <span className={styles.mobileCardLabel}>Тип:</span>
                        <span className={styles.mobileCardValue}>
                          {(() => {
                            switch (transaction.transactionType) {
                              ...
                              case 'base_to_bunker': return 'База → Бункеровщик';
                              case 'bunker_to_base': return 'Бункеровщик → База';
                      ...
                      <div className={styles.mobileCardField}>
                        <span className={styles.mobileCardLabel}>Количество:</span>
                        <span className={styles.mobileCardValue}>{transaction.quantity.toFixed(2)} л</span>
                      </div>
                      <div className={styles.mobileCardField}>
                        <span className={styles.mobileCardLabel}>Цена:</span>
                        <span className={styles.mobileCardValue}>{transaction.price?.toFixed(2) || '-'} ₽/л</span>
                      </div>
                      <div className={styles.mobileCardField}>
                        <span className={styles.mobileCardLabel}>Сумма:</span>
                        <span className={styles.mobileCardValue}>{((transaction.price || 0) * transaction.quantity).toFixed(2)} ₽</span>
                      </div>
                      <div className={styles.mobileCardField}>
                        <span className={styles.mobileCardLabel}>Дата:</span>
                        <span className={styles.mobileCardValue}>{dayjs(transaction.date).format('DD.MM.YYYY HH:mm')}</span>
                      </div>
                    </div>
                  </div>
                ))}
