            ...
            {filteredOrders.map((order) => (
              <div key={order.id} className={styles.orderCard}>
                <div className={styles.orderCardHeader}>
                  <span className={styles.orderCardTitle}>Заказ #{order.id}</span>
                  <div className={styles.orderCardActions}>
                    <Button size="small" icon={<EditOutlined />} onClick={() => handleEditOrder(order)} />
                ...
                <div className={styles.orderCardContent}>
                  <div className={styles.orderCardField}>
                    <span className={styles.orderCardLabel}>Клиент:</span>
                    <span className={styles.orderCardValue}>{order.customerName}</span>
                  </div>
                  <div className={styles.orderCardField}>
                    <span className={styles.orderCardLabel}>Топливо:</span>
                    <span className={styles.orderCardValue}>{order.fuelType}</span>
                  </div>
                  <div className={styles.orderCardField}>
                    <span className={styles.orderCardLabel}>Количество:</span>
                    <span className={styles.orderCardValue}>{order.quantity} л</span>
                  </div>
                  <div className={styles.orderCardField}>
                    <span className={styles.orderCardLabel}>Цена:</span>
                    <span className={styles.orderCardValue}>{order.price} ₽/л</span>
                  </div>
                  <div className={styles.orderCardField}>
                    <span className={styles.orderCardLabel}>Сумма:</span>
                    <span className={styles.orderCardValue}>{(order.quantity * order.price).toFixed(2)} ₽</span>
                  </div>
                  <div className={styles.orderCardStatus}>
                    <Tag color={order.status === 'new' ? 'blue' : order.status === 'processing' ? 'orange' : 'green'}>
