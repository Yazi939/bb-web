  ...
  try {
    ...
    const transactionData = transactions.map((item, index) => {
      ...
      switch(item.type) {
        ...
        case 'drain': typeLabel = 'Слив'; break;
    ...
    const summaryData = [
      ...
      { 'Показатель': 'Слито топлива (л)', 'Значение': totalDrained },
      { 'Показатель': 'Остаток топлива (л)', 'Значение': totalPurchased - totalSold - totalDrained },
